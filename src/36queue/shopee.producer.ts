...
export class ShopeeProducer {
  ...
  async publishBatchOrders(orders: string[][]) {
    ...
    this.bufferedWriteLog(`s_unconfirmed${this.id}.log`, this.counter_unconfirmed);
    this.bufferedWriteLog(`s_confirmed${this.id}.log`, this.counter_confirmed);
    console.log(`Batch of Shopee ${orders.length} orders published successfully`);
  }
...
export async function runShopeePub(id: string) {
  const producer = new ShopeeProducer(id);
