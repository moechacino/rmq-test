...
export class LazadaProducer {
  ...
  async publishBatchOrders(orders: string[][]) {
    ...
    this.bufferedWriteLog(`l_unconfirmed${this.id}.log`, this.counter_unconfirmed);
    this.bufferedWriteLog(`l_confirmed${this.id}.log`, this.counter_confirmed);
    console.log(`Batch of Lazada ${orders.length} orders published successfully`);
  }
...
export async function runLazadaPub(id: string) {
  const producer = new LazadaProducer(id);
